...
The session then loops forever: block until a new issue is opened on that repo →
implement the fix on a `bugbuster/issue-N` branch → run the repo's own tests/build →
push → open a PR with `Fixes #N` → squash-merge → confirm the issue closed → wait for
the next ticket. Tickets that arrive while a fix is in progress are queued and picked
up next. Unclear tickets get a comment asking for detail instead of code changes.
...
~/.bugbuster/bin/bugbuster status         # list listeners and queue depths
```
